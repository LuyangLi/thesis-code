...
int wod_get(wod_data_t * data) {
	...
	current_in = wod_eps_hk.curin[0] +wod_eps_hk.curin[1] + wod_eps_hk.curin[2];
	current_in = (uint16_t)floor((double)current_in * (255.0 / 2700.0));

	current_out = (uint16_t)floor((double)wod_eps_hk.cursys * (255.0 / 4000.0));
	...
	data->current_in = int16_to_uint8(current_in);
	data->current_out = int16_to_uint8(current_out);
