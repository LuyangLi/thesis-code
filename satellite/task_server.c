...
void vWork(csp_conn_t * conn, int timeout) {
	...
	while ((packet = csp_read(conn, timeout)) != NULL) {
		...
		//Luyang: record down the last ground communication time here.
		if (csp_conn_src(conn) > 7)
		...
		case OBC_PORT_LOAD_IMG: {
			...
			if (fd < 0) {
				...
				printf("this error is in task_server.\r\n");
				csp_buffer_free(packet);
				break;
			}
		...
		/* Custom tasks Ended.*/
		/* Don't know why this case is here, should put it here in order
			to make it fall through.
		*/
